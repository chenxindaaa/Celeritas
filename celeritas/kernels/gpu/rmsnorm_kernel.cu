...
void rmsKernelCu(const eUTIL::Tensor<float>& input, const eUTIL::Tensor<float>& weight,
    ...
    float* out_ptr = const_cast<float*>(output.data());
    constexpr int blockSize = 128;
    if (stream) {
        cudaStream_t stream_ = static_cast<cudaStream_t>(stream);
        row_rmsnorm_f32<128><<<1, blockSize, 0, stream_>>>(in_ptr, wei_ptr, out_ptr, size, eps);
    } else {
        row_rmsnorm_f32<128><<<1, blockSize>>>(in_ptr, wei_ptr, out_ptr, size, eps);
    }
}
