...
TEST(test_load, load_model_config) {
    ...
    ASSERT_EQ(config.hiddenDim, 768);
    ASSERT_EQ(config.layerNum, 6);
    ASSERT_EQ(config.headNum, 6);
    ASSERT_EQ(config.kvHeadNum, 6);
    ASSERT_EQ(config.vocabSize, 32000);
    ASSERT_EQ(config.seqLen, 256);
}
...
TEST(test_load, load_model_weight) {
    ...
    for (int i = 0; i < config.dim * config.hiddenDim; ++i) {
...
//   void* data = mmap(nullptr, file_size, PROT_READ, MAP_PRIVATE, fd, 0);
//   float* weight_data =
//       reinterpret_cast<float*>(static_cast<int8_t*>(data) + sizeof(ModelConfig));
...
//                                               config.hiddenDim, false);
//   float* in = new float[config.hiddenDim];
...
//     out[i] = 0.f;
//   }
//   tensor::Tensor tensor(base::DataType::kDataTypeFp32, config.hiddenDim, false, nullptr, in);
...
//   wq->set_weight(0, {config.dim, config.hiddenDim}, weight_data, base::DeviceType::kDeviceCPU);
//   wq->forward(); // 完成一个计算
